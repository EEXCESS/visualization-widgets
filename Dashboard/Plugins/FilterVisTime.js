(function(){
        ...
        if(allData === (null || undefined)){
            ...
            if($vis.length === 0){
                    ...
                    focus = svg.append("g")
                                ...
                                .attr("width", "100%" )
                                .attr("height", dataSet.newSize);
                    generateTimeline(allData, dataSet, value, fromYear, toYear,svg,focus);
    ...
    generateTimeline = function(allData, data, category, fromYear, toYear, svg, foc){
        deleteElements(foc);
        var dataSet = data;
        SCALE = dataSet.scaleX;
        var mainframe = d3.select("FilterVisTime").attr('height', dataSet.newSize);
     ...
     * 
     */ 
    interactTimelineTranslate = function(fromYear, toYear, mainframe, dataSet) {
     ...
     */
	calcTransitionPoints  = function (coordinates, translate, fromYear, toYear, size, sizeStart, start, end, centre){
     ...
     */
	xAxisDenseFunction = function(mainframe, start, end, fromYear, toYear, data){
     ...
     * 
     */
	generateTicks  = function(elements, segmentStartX, segmentEndX, lineY, focus, end, one){
	    var xAxis = [];
     ...
     */ 
    calcTicks = function (start, end, fromYear, toYear){
        var scale = SCALE;
        var array = [];
        var year = [];
        for (var i = start; i < end; i++){
