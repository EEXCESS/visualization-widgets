function facetScape(domElem, iwidth, iheight, ifacets, queryResultItems, term) {
    ...
    // Animation
    var FACET_POLYGON_COLLISION_COLOR = "#008746";
    ...
    var RENDERING = (function() {
        ...
        return {
            drawResultList: function() {
                ...
                var singleResultNode = resultNodes.enter().append("div").attr("class", "resultList-item");
                ...
                var secDesc = singleResultNode.append("div").attr("class", "resultList-item-description")
                    .text(function(d, i) {
                        ...
                        if(d['provider'] && d['provider'] != "unknown") {
                            desc.push((d['provider'] instanceof Array) ? d['provider'].join(", ") : d['provider']);
                        }
                        if(d['date'] && d['date'] != "unknown") {
                            desc.push((d['date'] instanceof Array) ? d['date'].join(", ") : d['date']);
                        }
                        if(d['language'] && d['language'] != "unknown") {
                            desc.push((d['language'] instanceof Array) ? d['language'].join(", ") : d['language']);
                        }
                        return desc.join(" - ");
                    });
            },
                ...
                if (item.hasOwnProperty(facetName)) {
                    if (item[facetName] instanceof Array) {
                        for (var i = 0; i < item[facetName].length; i++) {
                            if (item[facetName][i] == tag) {
                                return true;
                            }
                        }
                        return false;
                    } else {
                        if (item[facetName] == tag) {
                            return true;
                        } else {
                            return false;
                        }
                            ...
                            if (selectedResults[i].hasOwnProperty(key)) {
                                if (selectedResults[i][key] instanceof Array) {
                                    for (var tt = 0; tt < selectedResults[i][key].length; tt++) {
                                        var canonTag = internal.getCanonicalString(selectedResults[i][key][tt]);
                                        if (canonTag == internal.getCanonicalString(facetData[f].tags[t].word)) {
                                            facetData[f].tags[t].frequency++;
                                        }
                                    }
                                } else {
                                    var canonTag = internal.getCanonicalString(selectedResults[i][key]);
                                    if (canonTag == internal.getCanonicalString(facetData[f].tags[t].word)) {
                                        facetData[f].tags[t].frequency++;
                                    }
                                }
